...
This confirms that if we provide a slightly higher voltage to the VCAP output, the regulator will not operate since it is being fed a voltage that exceeds its threshold from the VCAP feedback loop. In fact, this is a timeless method (that is removing an external, regulator-connected capacitor and injecting power to disable regulation) that is used quite frequently. Although I cannot find the original mention of this method it seems to point closely to <a href="https://archives.sector.ca/presentations19/2019_Thomas_Roth_chip-fail.pdf">chip.fail</a>. The next question we have is naturally, how do we find this perfect voltage without damaging the chip? I'll go over this in a simple method called "regulator sweeping". Although I just made up the name for this post, I am confident it has been done a thousand times. We now have a way to bypass the regulator and have enough information to start drafting (and I recommend you draft first) a measurement circuit. The most important part is the VCAP pad where the 2.2uf decoupling capacitor used to be. It will be responsible for the following:
...
- chip.fail - [chip.fail](https://archives.sector.ca/presentations19/2019_Thomas_Roth_chip-fail.pdf)
